...
class CourseControllerTest {
    ...
    @Test
    @DisplayName("POST /api/v1/courses → 201 Created with Location header")
    ...
    @Test
    @DisplayName("POST /api/v1/courses with blank courseName → 400")
    ...
    @Test
    @DisplayName("POST /api/v1/courses with credits=0 → 400")
    ...
    @Test
    @DisplayName("POST /api/v1/courses with duplicate ID → 409 Conflict")
    ...
    @Test
    @DisplayName("GET /api/v1/courses → 200 OK with course list")
    void getAllCourses_returnsList() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/courses"))
                ...
                .andExpect(jsonPath("$._embedded.courses.length()").value(2))
                .andExpect(jsonPath("$._embedded.courses[0].id").value("CS101"))
                .andExpect(jsonPath("$._links.self.href", endsWith("/api/v1/courses")));
    }

    @Test
    @DisplayName("GET /api/v1/courses with no data → 200 OK empty collection with self link")
    ...
    @Test
    @DisplayName("GET /api/v1/courses/{id} → 200 OK when course exists")
    ...
    @Test
    @DisplayName("GET /api/v1/courses/{id} → includes HATEOAS self, collection, and related links")
    void getCourseById_includesLinks() throws Exception {
        when(courseService.getCourseByIdDTO("CS101")).thenReturn(sampleCourse);

        mockMvc.perform(get("/api/v1/courses/CS101"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._links.self.href", endsWith("/api/v1/courses/CS101")))
                .andExpect(jsonPath("$._links.courses.href", endsWith("/api/v1/courses")))
                .andExpect(jsonPath("$._links.enroll.href", endsWith("/api/v1/courses/CS101/enroll")))
                .andExpect(jsonPath("$._links.unenroll.href", endsWith("/api/v1/courses/CS101/unenroll")));
    }

    @Test
    @DisplayName("GET /api/v1/courses → each item carries its own self link")
    void getAllCourses_itemsHaveLinks() throws Exception {
        when(courseService.getAllCoursesDTO()).thenReturn(List.of(sampleCourse, fullCourse));

        mockMvc.perform(get("/api/v1/courses"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.courses[0]._links.self.href", endsWith("/api/v1/courses/CS101")))
                .andExpect(jsonPath("$._embedded.courses[1]._links.self.href", endsWith("/api/v1/courses/CS999")));
    }

    @Test
    @DisplayName("POST /api/v1/courses → created course includes a self link")
    void createCourse_includesSelfLink() throws Exception {
        when(courseService.createCourseDTO(any())).thenReturn(sampleCourse);
    ...
    @Test
    @DisplayName("GET /api/v1/courses/{id} → 404 when course does not exist")
    ...
    @Test
    @DisplayName("PUT /api/v1/courses/{id} → 200 OK when update succeeds")
    void updateCourse_success() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/courses/CS101")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("PUT /api/v1/courses/{id} → 404 when course does not exist")
    void updateCourse_notFound() throws Exception {
        when(courseService.updateCourse(eq("MISSING"), any()))
                .thenThrow(new CourseNotFoundException("Course not found: MISSING"));

        mockMvc.perform(put("/api/v1/courses/MISSING")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(sampleCourse)))
                .andExpect(status().isNotFound());
    }
    ...
    @Test
    @DisplayName("DELETE /api/v1/courses/{id} → 204 No Content on success")
    void deleteCourse_success() throws Exception {
        doNothing().when(courseService).deleteCourseOrThrow("CS101");

        mockMvc.perform(delete("/api/v1/courses/CS101"))
    ...
    @Test
    @DisplayName("DELETE /api/v1/courses/{id} → 404 when course does not exist")
    void deleteCourse_notFound() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/courses/GONE"))
                .andExpect(status().isNotFound());
    }

    @Test
    @DisplayName("POST /api/v1/courses/{id}/enroll → 200 OK with updated enrolledStudents")
    void enrollStudent_success() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/courses/CS101/enroll"))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("POST /api/v1/courses/{id}/enroll → 400 when course is full")
    void enrollStudent_courseFull() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/courses/CS999/enroll"))
                .andExpect(status().isBadRequest());
    }

    @Test
    @DisplayName("POST /api/v1/courses/{id}/unenroll → 200 OK with updated enrolledStudents")
    void unenrollStudent_success() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/courses/CS101/unenroll"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.enrolledStudents").value(9));
    }

    @Test
    @DisplayName("POST /api/v1/courses/{id}/unenroll → 400 when no students enrolled")
    ...
    @Test
    @DisplayName("GET /api/v1/courses/available → 200 OK with only non-full courses")
    void getAvailableCourses() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/courses/available"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.courses.length()").value(1))
                .andExpect(jsonPath("$._embedded.courses[0].id").value("CS101"));
    }

    @Test
    @DisplayName("GET /api/v1/courses/search?name=java&minCredits=3 → 200 OK filtered results")
    void searchCourses_withBothParams() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/courses/search")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.courses.length()").value(1))
                .andExpect(jsonPath("$._embedded.courses[0].courseName").value("Intro to Java"));
    }
}
